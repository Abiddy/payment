...
Our database currently uses the `transactions` table (as mentioned in section 1) which stores both estimated and actual fee values. For future implementation, we'll add `streamer_balances` (performance cache for balances as mentioned in section 3), `payouts` (tracks payout history as mentioned in section 3), and `points_ledger` (only if points system is implemented as mentioned in section 2).
