...
Streamer Accoiunt Balances can be managed in 3 states: 
**Pending Balance** (tips not yet cleared, held for 7 days for fraud prevention)
**Available Balance** (money cleared and ready for payout)
**Paid Out** (money transferred to streamer's bank account). 

Balances can be calculated from the existing `transactions` table by summing `streamer_amount_actual` for succeeded transactions, with pending balance including transactions within the 7-day hold period and available balance including older transactions. I suggest maintaining a `streamer_balances` table as a performance cache with fields: `streamer_id`, `pending_balance`, `available_balance`, `total_paid_out`, and `updated_at`. This table can be updated in real-time when transactions succeed (via webhook) and when payouts are processed.

In Stripe, I suggest identifying streamer transactions using metadata with `streamer_id`, while in our database we can use `streamer_id` as the primary identifier. I suggest tracking payouts in a separate `payouts` table with fields: `payout_id`, `streamer_id`, `amount`, `stripe_payout_id` (links to Stripe's payout record), `status` ('pending', 'paid', 'failed'), `created_at`, and `paid_at`. When a payout is created, we can deduct from `available_balance` and create a `payouts` record. When Stripe confirms the payout via webhook, we can update the `payouts.status` to 'paid' and add to `total_paid_out`. Both cash tips and point tips can contribute to the same balance—point tips can convert to cash immediately when tipped (as per our design), and both can create entries in the `transactions` table with the same `streamer_id`, so they can be aggregated together in the balance calculation.

Streamer balances can be displayed in a streamer dashboard showing `pending_balance`, `available_balance`, and payout history. I suggest managing balances in our database (as our source of truth), while Stripe can be used for actual payout execution. I suggest reconciling daily by comparing `available_balance + total_paid_out` against Stripe's transfer records, matching `payouts.stripe_payout_id` with Stripe payout objects, and flagging any discrepancies for manual review.
...
We can implement our own API endpoints using Next.js API routes! Currently working endpoints include: `POST /api/create-payment-intent` (creates Payment Intent and stores transaction with estimated fees as mentioned in section 1), `POST /api/webhook` (handles `payment_intent.succeeded`, `payment_intent.payment_failed`, `payment_intent.canceled` and fetches actual Stripe fees as mentioned in section 1), `GET /api/transactions` (returns all transactions with statistics), `GET /api/streamers`, and `GET /api/check-payment`. For future implementation, we'll add `GET /api/balance` and `POST /api/payout-request` for payout functionality (as mentioned in section 3). We'll also need to handle `charge.dispute.created` and `payout.paid` webhook events (as mentioned in sections 3 and 4).

Our database currently uses the `transactions` table as shown above, which stores both estimated and actual fee values. For future implementation, we can add `streamer_balances` (performance cache for balances as mentioned in section 3), `payouts` (tracks payout history as mentioned in section 3), and `points_ledger` (only if points system is implemented as mentioned in section 2).

We prevent duplicate transactions by checking for existing records by `payment_id` before creating new ones—if found, we update the existing transaction instead of creating a duplicate. Our webhook handler also checks for existing transactions before processing to ensure idempotency. For future payouts, we'll use Stripe's idempotency keys and check the `payouts` table for duplicate requests within a short time window, with database-level unique constraints on `stripe_payout_id` to ensure each Stripe payout is only recorded once.
